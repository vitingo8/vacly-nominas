...
interface GenerationRequest {
  ...
  /** Si true, calcula y genera PDF en memoria sin persistir en Supabase. */
  dryRun?: boolean
}

export type PayrollGenerationResultItem = {
  employeeId: string
  employeeName: string
  success: boolean
  nominaId?: string
  ...
  error?: string
  /** Solo en dryRun: PDF generado en memoria. */
  pdfBytes?: Uint8Array
  /** Solo en dryRun: registro que se habría insertado en `nominas`. */
  nominaPreview?: Record<string, unknown>
...
export async function runPayrollGeneration(
  ...
  options: { supabase?: ReturnType<typeof getSupabaseClient> } = {},
) {
  try {
  ...
  const { companyId, companyData, month, year, employees } = body
    ...
    if (!companyId || !month || !year || !employees || employees.length === 0) {
      return {
        success: false,
        error: 'Faltan campos requeridos: companyId, month, year, employees',
        status: 400,
      }
    }
    ...
    if (!dryRun && (year > currentYear || (year === currentYear && month >= currentMonth))) {
      return {
        success: false,
        error: 'No se pueden generar nóminas para el mes actual ni meses futuros. Solo se permiten meses ya cerrados.',
        status: 400,
      }
    }
    ...
    const results: PayrollGenerationResultItem[] = []
    ...
    for (const emp of employees) {
      try {
        ...
        // Sin convenio derivado, la prorrata del input (o base×pagas/12) debe
        // entrar como devengo salarial, no solo en base CC.
        const monthlyBonusAccrual = derived
          ? derived.monthlyProratedBonuses
          : round2(
              Number(emp.proratedBonuses) > 0
                ? Number(emp.proratedBonuses)
                : (effectiveBase * effectiveNumberOfBonuses) / 12,
            )
        ...
        let savedNomina: { id: string } | null = null
        if (!dryRun) {
        const { data: insertedNomina, error: saveError } = await supabase
          .from('nominas')
          .insert(nominaRecord)
          .select('id')
          .single()
        ...
        savedNomina = insertedNomina
        }

        // Generate PDF payslip
        let generatedPdfBytes: Uint8Array | undefined
        let pdfData: PayslipPDFData | undefined
        try {
    ...
    return {
      ...
      dryRun,
      message: dryRun
        ? `${successCount} nómina(s) calculada(s) en modo prueba${errorCount > 0 ? `, ${errorCount} error(es)` : ''}`
        : `${successCount} nómina(s) generada(s) correctamente${errorCount > 0 ? `, ${errorCount} error(es)` : ''}`,
      results,
      summary: {
    ...
    console.error('runPayrollGeneration error:', error)
    return {
      success: false,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outcome = await runPayrollGeneration(body)
