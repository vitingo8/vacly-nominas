...
export async function POST(request: NextRequest) {
  try {
    console.log('🔄 Starting PDF processing with progress tracking...')
      ...
      start(controller) {
        ...
        (async () => {
          try {
            ...
            sendProgress(15, 'PDF descargado, analizando documento...')
            ...
            sendProgress(25, `Documento analizado: ${pageCount} páginas encontradas. Iniciando división...`, 0, pageCount)

            const documents: SplitDocument[] = []

            // Process each page with real-time progress updates
            for (let i = 0; i < pageCount; i++) {
              const pageNum = i + 1
              const pageId = uuidv4()
              
              // Calculate progress: 25% (initial setup) + 70% (pages processing) + 5% (completion)
              const pageProgress = 25 + Math.round((i / pageCount) * 70)
              sendProgress(pageProgress, `Procesando página ${pageNum} de ${pageCount}...`, pageNum, pageCount)
              ...
              try {
                ...
                if (pdfUploadError) {
                  ...
                  // REMOVED continue - now always shows documents
                }
                ...
                if (processedDocError) {
                  ...
                  // // REMOVED continue - now always shows documents with processing even if database insert fails
                } else {
                ...
                // Update progress after each page is completed
                const completedProgress = 25 + Math.round(((i + 1) / pageCount) * 70)
                sendProgress(completedProgress, `Página ${pageNum} completada (${pageBasicInfo.employeeName})`, pageNum, pageCount)

              } catch (pageError) {
                console.error(`❌ Error processing page ${pageNum}:`, pageError)
                // // REMOVED continue - now always shows documents with next page
              }
            }

            sendProgress(100, `¡Procesamiento completado! ${documents.length} documentos creados`)
