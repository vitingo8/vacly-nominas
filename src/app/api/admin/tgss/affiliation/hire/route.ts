...
import type { AffiliationRequestType } from '@/lib/admin-integrations/types'

async function handleAffiliation(request: NextRequest, requestType: AffiliationRequestType) {
  try {
    ...
    if (!companyId || !employeeId) {
      return adminErrorResponse(
        new Error('company_id y employee_id son requeridos'),
      )
    }
...
export async function POST(request: NextRequest) {
  return handleAffiliation(request, 'alta')
}
