-- ============================================================================
-- 20260432_fn_v3_resolve_salary_base_annual_retribution_fallback.sql
--
-- Muchos convenios no tienen filas con key LIKE 'salary_table_%' sino
-- tablas tipo annual_retribution_example_* con filas:
--   { "concepto": "Salario base", "importe_mensual": ... }
-- sin grupo/nivel por fila; el perfil va implícito en el `key` de la tabla
-- (ej. personal_limpiador).
--
-- Esta versión UNION esas filas y las rankea por tokens de p_categoria (>=5
-- caracteres) presentes en lower(t.key). Si no hay tokens, acepta cualquier
-- fila "Salario base" del anexo (último recurso).
-- ============================================================================
...
AS $$
  WITH tokens AS (
    ...
    FROM regexp_split_to_table(
      lower(regexp_replace(coalesce(p_categoria, ''), '[^a-záéíóúñ0-9]+', ' ', 'g')),
      ' '
    ) AS tok
