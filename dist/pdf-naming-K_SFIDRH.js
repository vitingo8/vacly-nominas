 ...
 */
async function extractBasicNominaInfoFromText(textContent) {
    try {
        if (!process.env.ANTHROPIC_API_KEY) {
            throw new Error('Anthropic API key not configured');
        }
 ...
 */
function generateGlobalFileName(companyName, period) {
    return `${period}_${companyName}.pdf`;
}
/**
 * Genera el nombre del archivo split
 */
function generateSplitFileName(employeeName, period, pageNumber) {
    return `${period}_${employeeName}.pdf`;
}
/**
 * Genera el nombre del archivo de texto
 */
function generateTextFileName(employeeName, period, pageNumber) {
