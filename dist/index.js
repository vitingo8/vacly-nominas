...
const supabase$1 = supabaseJs.createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
    ...
    async learnFromDocument(companyId, employeeId, documentData, confidence = 0.75) {
        try {
            ...
            const memoryRecord = {
                ...
                document_type_id: documentTypeId,
                conversation_id: uuid.v4(), // Generate a conversation ID for this memory session
                summary: summary,
...
const voyageClient = process.env.VOYAGE_API_KEY ? new voyageai.VoyageAIClient({
...
const supabase = supabaseJs.createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
...
function createNominaProcessor(config) {
    return {
        extractBasicInfo: (content) => typeof content === 'string'
            ? pdfNaming.extractBasicNominaInfoFromText(content)
            : pdfNaming.extractBasicNominaInfo(content),
        generateFileName: (employeeName, period, pageNumber) => pdfNaming.generateSplitFileName(employeeName, period),
...
exports.VACLY_VERSION = VACLY_VERSION;
exports.cn = cn;
...
exports.storeDocumentEmbeddings = storeDocumentEmbeddings;
