...
import { generateQueryEmbedding, chunkText, generateEmbeddings, calculateVoyageCost } from './embeddings';
const supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
// Buscar documentos similares usando embeddings de Voyage AI
export async function findSimilarDocuments(queryText, companyId, documentType, employeeId, limit = 5, threshold = 0.7) {
    try {
...
export async function getMemoryContext(companyId, documentType, employeeId) {
    try {
        ...
        const learnedKeywords = Array.from(new Set(allPatterns.flatMap(p => p.keywords || [])));
        return {
            similar_documents: [],
...
export async function storeDocumentEmbeddings(documentId, companyId, documentTypeId, extractedText, employeeId) {
    try {
...
export async function updateMemory(companyId, documentTypeId, processedData, conversationId, employeeId) {
    try {
...
export async function buildClaudeContext(queryText, companyId, documentType, employeeId) {
    try {
